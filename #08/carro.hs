...
createCar a b = Info (Car {pass = 0, maxPass = a, gas = 0, maxGas = b, km = 0}) (Op "Criar Carro" True)

-- enche o tanque passando a qtd de gas. Retorna falso apenas se o tanque já estiver completamente cheio.
fuel :: Int -> Info -> Info
fuel a (Info (Car pass maxPass gas maxGas km) op)
    | gas < maxGas = Info (Car pass maxPass (x - y) maxGas km) (Op "Encher Tanque" True )
    | otherwise =  Info (Car pass maxPass gas maxGas km) (Op "Encher Tanque" False )
    where
        y = if x > maxGas then x - maxGas else 0
        x = gas + (abs a)

-- Faz entrar uma pessoa no carro. Retorna false se já estiver lotado.
embark :: Info -> Info
embark (Info (Car pass maxPass gas maxGas km) op) = Info (Car newPass maxPass gas maxGas km) (Op "Embarcar" sucess )
    where
        sucess = if newPass == pass then False else True
        newPass = if pass < maxPass then (pass + 1) else pass

-- Retira uma pessoa do carro, retorna false se não tiver ninguém no carro
disembark :: Info -> Info
disembark (Info (Car pass maxPass gas maxGas km) op)
    | pass > 0 = Info (Car (pass - 1) maxPass gas maxGas km) (Op "Desembarcar" True )
    | otherwise = Info (Car pass maxPass gas maxGas km) (Op "Desembarcar" False )
...
drive x (Info (Car pass maxPass gas maxGas km) op) 
    | pass == 0 || gas == 0 = Info (Car pass maxPass gas maxGas km) (Op "Dirigir" False)
    | otherwise = Info (Car pass maxPass newGas maxGas newKm) (Op "Dirigir" checkFullRun)
    where
      newKm = if newGas > 0 then km + x else km + gas
      newGas = if x > gas then 0 else gas - x 
      checkFullRun = if gas < x then False else True
